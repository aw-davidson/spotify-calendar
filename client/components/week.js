...
const Week = (props) => {
  ...
  for (let i = 0; days.length < 7; currentDay = currentDay.clone().add(1, "d")) {
    ...
    days.push(<Day key={currentDay.format("YYYY-MM-DD")}
      selected={selected} displayMonth={displayMonth} currentDay={currentDay} handleDayClick={handleDayClick} currentDayEvents={currentDayEvents} deleteEvent={deleteEvent}
    />
    );
  }
